...
#if DS == 0
	const double *d1 = (const double *)double1,
		     *d2 = (const double *)double2;
	double diff = *d1 - *d2;
#elif DS == 1
	const double **d1 = (const double **)double1,
		     **d2 = (const double **)double2;
...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < row_size; i++) {
		...
		double c_price_diff = (row->c_price - pre_row->c_price) /
				      pre_row->c_price * 100;
	...
	for (int i = 0; i < row_size; i++) {
		...
		double intraday_ret =
			(row->c_price - row->o_price) / row->o_price * 100;
	...
	uint32_t double_data_size = double_data.retrows(&double_data);
	for (int i = 0; i < double_data_size; i += 4) {
		row = set_obj.getrow(&set_obj, i >> 2);
		double_data.setrow(&double_data, i + 0, &row->o_price);
		double_data.setrow(&double_data, i + 1, &row->h_price);
		double_data.setrow(&double_data, i + 2, &row->l_price);
		double_data.setrow(&double_data, i + 3, &row->c_price);
	}

	double_data.sort(&double_data, compare_double);

	double d_min, d_max, d_median1, d_median2, *d;
	d = double_data.getrow(&double_data, 0);
	d_min = *d;
	d = double_data.getrow(&double_data, double_data_size - 1);
	d_max = *d;
	d = double_data.getrow(&double_data, double_data_size >> 1);
	d_median1 = *d;
	d = double_data.getrow(&double_data, (double_data_size >> 1) + 1);
	d_median2 = *d;
